...
for i in range(vezes):
    ...
    nome = st.text_input("Nome", icon="📝", placeholder="Digite o nome...")
    idade = st.number_input("Idade", icon="🔢", placeholder="Digite a idade...", min_value=1)
    altura = st.number_input("Altura (em metros)", icon="📏", placeholder="Digite a altura (em metros)...", min_value=0.01, max_value=2.80)
...
if (i+1) == quantidadePessoas:
    ...
    st.text(f"A altura média dos participantes é de {sum(listaAlturas) / i+1}")
